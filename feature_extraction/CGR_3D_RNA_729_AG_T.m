...
for ipp = 1:length(pp)
    ...
    for ind = 1:length(seq)
        ...
        eval(['imwrite(img, "CGRxy_3D_RNA_729_AG_T\CGRxy_3D_729_', num2str(ind), '.png");']); % 保存到工作目录下，名字为"a.png"
        close all
        ...
        img = frame2im(frame); % 将frame变换成imwrite函数可以识别的格式
        eval(['imwrite(img, "CGRxz_3D_RNA_729_AG_T\CGRxz_3D_729_', num2str(ind), '.png");']); % 保存到工作目录下，名字为"a.png"
        close all
        ...
        img = frame2im(frame); % 将frame变换成imwrite函数可以识别的格式
        eval(['imwrite(img, "CGRyz_3D_RNA_729_AG_T\CGRyz_3D_729_', num2str(ind), '.png");']); % 保存到工作目录下，名字为"a.png"
        close all
    end
end
toc