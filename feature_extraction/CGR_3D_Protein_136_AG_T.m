...
clear
tic
[name, seq] = fastaread("NPInter_rdprotein_seq_136.fasta");
for i = 1:136
    len(i) = length(seq{i});
end
lenmax = max(len);
...
for ipp = 1:length(pp)
    ...
    for ind = 1:length(seq)
        ...
        [xo, yo, zo] = cgr3dprotein_AG_T(data);
        x = mapminmax(xo, 0, 1);
        y = mapminmax(yo, 0, 1);
        z = mapminmax(zo, 0, 1);
        % 比标准序列长的
...
%         eval(['imwrite(img, "CGRxy_3D_Protein_136_', num2str(lenstd),'_AG_T\CGRxy_3D_136_', num2str(lenstd),'_', num2str(ind), '.png");']); % 保存到工作目录下，名字为"a.png"
%         close all
        ...
        img = frame2im(frame); % 将frame变换成imwrite函数可以识别的格式
        eval(['imwrite(img, "CGRxz_3D_Protein_136_', num2str(lenstd),'_AG_T\CGRxz_3D_136_', num2str(lenstd),'_', num2str(ind), '.png");']); % 保存到工作目录下，名字为"a.png"
        close all
        ...
        img = frame2im(frame); % 将frame变换成imwrite函数可以识别的格式
        eval(['imwrite(img, "CGRyz_3D_Protein_136_', num2str(lenstd),'_AG_T\CGRyz_3D_136_', num2str(lenstd),'_', num2str(ind), '.png");']); % 保存到工作目录下，名字为"a.png"
        close all
    end
end
toc